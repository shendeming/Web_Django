...
def index(request, pIndex=1):
    ...
    # 执行分页处理
    slist = smod.filter(status__lt=9)        # 过滤，剩下status<9的员工信息，因为status=9状态为删除
    
    mywhere = []                             # 定义存储搜索条件的mywhere用以维持搜索条件
    # 获取并判断搜索条件，封装keyword键搜索
    kw = request.GET.get("keyword",None)
    if kw:
        slist = slist.filter(name__contains=kw)  # 做模糊式查询，查询店铺名称中只要含有关键字就可
...
def insert(request):
    ...
    try:
        ...
        destination = open("./static/uploads/shop/"+cover_pic,"wb+")      # 确保上传后的图片名称不同
